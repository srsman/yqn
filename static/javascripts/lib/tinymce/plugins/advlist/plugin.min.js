
tinymce.PluginManager.add("advlist", function (a) {
  var trans = {
    'default':'1 2 3 ...',
    'lower_alpha': 'a b c ...',
    'lower_greek':'α β γ ...',
    'lower_roman':'i ii iii ...',
    'upper_alpha':'A B C ...',
    'upper_roman':'Ⅰ Ⅱ Ⅲ ...',
    'circle':'○  一一一一',
    'disc':'●  一一一一',
    'square':'◆  一一一一'
  };
  function b(a, b) {
    var c = [];
    return tinymce.each(b.split(/[ ,]/), function (a) {
      var _a = a.replace(/\-/g,"_");
      c.push({
        //text: a.replace(/\-/g, " ").replace(/\b\w/g, function (a) {
        //  return a.toUpperCase()
        //}),
        text: trans[_a],
        data: "default" == a ? "" : a
      })
    }), c
    console.log(c);
  }

  function c(b, c) {
    a.undoManager.transact(function () {
      var d, e = a.dom, f = a.selection;
      d = e.getParent(f.getNode(), "ol,ul"), d && d.nodeName == b && c !== !1 || a.execCommand("UL" == b ? "InsertUnorderedList" : "InsertOrderedList"), c = c === !1 ? g[b] : c, g[b] = c, d = e.getParent(f.getNode(), "ol,ul"), d && (e.setStyle(d, "listStyleType", c ? c : null), d.removeAttribute("data-mce-style")), a.focus()
    })
  }

  function d(b) {
    var c = a.dom.getStyle(a.dom.getParent(a.selection.getNode(), "ol,ul"), "listStyleType") || "";
    b.control.items().each(function (a) {
      a.active(a.settings.data === c)
    })
  }

  var e, f, g = {};
  e = b("OL", a.getParam("advlist_number_styles", "default,lower-alpha,lower-greek,lower-roman,upper-alpha,upper-roman")), f = b("UL", a.getParam("advlist_bullet_styles", "circle,disc,square")), a.addButton("numlist", {
    type: "splitbutton",
    tooltip: "编号格式",
    menu: e,
    onshow: d,
    onselect: function (a) {
      c("OL", a.control.settings.data)
    },
    onclick: function () {
      c("OL", !1)
    }
  }), a.addButton("bullist", {
    type: "splitbutton", tooltip: "项目符号", menu: f, onshow: d, onselect: function (a) {
      c("UL", a.control.settings.data)
    }, onclick: function () {
      c("UL", !1)
    }
  })
});