...
router.get('/', function(req, res) {
  ...
  getListParams['req'] = req;
  getListParams['res'] = res;
  getListParams['areaId'] = areaId;
  getListParams['brokerId'] = brokerId;
  getListParams['currPage'] = currPage;
  getListParams['newStockUrl'] = newStockUrl;
  getListParams['newStockParams'] = newStockParams;
  getListParams['brokerListUrl'] = brokerListUrl;
  getListParams['brokerListParams'] = brokerListParams;

  getBrokerList(getListParams);
});
 ...
 * @qiulijun
 */
router.get('/:area/:broker', function(req, res) {
  var newStockUrl = '/mktinfo_api/fetch_ipo_list',
      brokerListUrl = '/seo_api/fetch_area_broker_list',
      brokerListParams = {},
      newStockParams = {};
...
router.get('/:area/:broker/:dept', function(req, res, next) {
  ...
  ajax.map.post({
    ...
    if(newStockData.code === 0 && brokerData.code === 0) {
      ...
      seoCustomMeta['title'] = fullName + ' - ' + appConfig.seo.broker.title;
      ...
      seoCustomMeta['description'] = appConfig.seo.broker.description + fullName + '网上开户，' + fullName + '怎么去？' + fullName + '佣金多少？' + fullName + '投资顾问，网上24小时在线预约股票开户，开户能享受什么服务？华泰证券投顾观点，券商软件下载，2.5佣金，最低开户佣金。';
...
function getBrokerList(getParams) {
  var newStockUrl = getParams.newStockUrl,
      newStockParams = getParams.newStockParams,
      brokerListUrl = getParams.brokerListUrl,
      brokerListParams = getParams.brokerListParams,
      areaId = getParams.areaId,
      brokerId = getParams.brokerId,
      currPage = getParams.currPage,
      req = getParams.req,
      res = getParams.res;
  ...
  ajax.map.post({
    ...
    if(newStockData.code === 0 && brokerListData.code === 0) {
      ...
      if(areaId > 0 && brokerId > 0) {
        ...
        if(brokerListData.result.busDepts.length > 0) {
        ...
        seoCustomMeta['description'] = appConfig.seo.broker.description + fullName + '网上开户，' + fullName + '营业部怎么去？' + fullName + '投资顾问，' + fullName + '怎么样？' + fullName + '的佣金多少？' + fullName + '在线交易，2.5佣金，最低开户佣金';
      } else if(brokerId > 0) {
        /* 券商纬度 */
        if(brokerListData.result.busDepts.length > 0) {
          brokerName = brokerListData.result.busDepts[0].brokerType;
        ...
        seoCustomMeta['title'] = brokerName + '营业部查询 - ' + appConfig.seo.broker.title;
        ...
        seoCustomMeta['description'] = appConfig.seo.broker.description + brokerName + '网上开户，' + brokerName + '怎么去？' + brokerName + '的佣金多少？' + brokerName + '投资顾问，' + brokerName + '哪个投顾厉害？网上24小时在线预约股票开户，开户能享受什么服务？' + brokerName + '投顾观点，券商软件下载，2.5佣金，最低开户佣金。';
      } else if(areaId > 0) {
        /* 地域纬度 */
        var areaBrokerName;
        ...
        seoCustomMeta['title'] = areaBrokerName + '营业部查询 - ' + appConfig.seo.broker.title;
        ...
        seoCustomMeta['title'] = '券商开户，营业部查询 - ' + appConfig.seo.broker.title;

        seoCustomMeta['keywords'] = appConfig.seo.broker.keywords;

        seoCustomMeta['description'] = appConfig.seo.broker.description + '券商软件下载，最低2.5开户佣金。';
      }
